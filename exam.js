...
const timer = setInterval(function () {
    let actualRemainingMinutes = String(Math.floor(remainingSeconds / 60));
    let actualRemainingSeconds = String(remainingSeconds % 60);

    if (actualRemainingMinutes.length < 2) actualRemainingMinutes = "0" + actualRemainingMinutes;
    if (actualRemainingSeconds.length < 2) actualRemainingSeconds = "0" + actualRemainingSeconds;
...
/******************************* SUBMIT ************************/
// Defined at top level so the timer above can call it
function submitExam() {
    clearInterval(timer);
    // e.g. redirect to results page:
...
async function initExam() {
    ...
    // ✅ Moved inside initExam so they are used only after state exists
    ...
    function updateSidebar() {
        ...
        if (markedQuestionIndexes.length === 0) {
            ...
            for (let i = 0; i < markedQuestionIndexes.length; i++) {
                ...
                const index = markedQuestionIndexes[i]; // ✅ Fix: capture correct index
    ...
    btnMark.addEventListener("click", toggleMark); // ✅ Fix: was missing event listener
